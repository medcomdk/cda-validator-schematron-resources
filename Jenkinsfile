podTemplate(
   ...
   volumes: [hostPathVolume(hostPath: '/var/run/docker.sock', mountPath: '/var/run/docker.sock')],
	...
	node(POD_LABEL) {
		...
		stage('Build image') {
         		container('docker') {
				docker.build("kvalitetsit/medcom-cda-validator-schematron-resources:${scmInfo.GIT_COMMIT}", "./schematron")
				}
      		}
		...
		stage('Tag Docker image and push to registry') {
			container('docker') {
				...
				docker.withRegistry('', 'dockerhub') {
					def image = docker.image("kvalitetsit/medcom-cda-validator-schematron-resources:${scmInfo.GIT_COMMIT}")
